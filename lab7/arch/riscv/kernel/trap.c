...
void trap_handler(uint64 scause, uint64 sepc, struct pt_regs* regs) {
    if (scause >> 63){ // 通过 `scause` 判断trap类型
        ...
        if (syscall_id == SYS_GETPID) {
            ...
            int fd = (int)regs->x[10];
            char* buffer = (char*)regs->x[11];
            uint64_t siz = (uint64_t)regs->x[12];

            ret = (uint64_t)sys_write(fd, buffer, siz);
            ...
            ret = (uint64_t)sys_read(fd, buffer, siz);
        } else if (syscall_id == SYS_LSEEK){
            int fd = (int)regs->x[10];
            int64_t offset = (int64_t)regs->x[11];
            int whence = (int)regs->x[12];

            ret = (uint64_t)sys_lseek(fd, offset, whence);
        } else if (syscall_id == SYS_CLOSE){
            int fd = (int)regs->x[10];
            ret = (uint64_t)sys_close(fd);
        } else if (syscall_id == SYS_OPENAT) {
            int dfd = (int)regs->x[10];
            char *filename = (char *)regs->x[11];
            int flags = (int)regs->x[12];

            ret = (uint64_t)sys_openat(dfd, filename, flags);
        } else {
